...
public class BaseExponentiation extends SingleIntOperation {
	...
	private List<Integer> filterAllThoseMatch (final Set<Integer> exponentCandidates) {
		...
		for (Integer exponentCandidate : exponentCandidates) {
			...
			for (int i = 0; i < hypotheses.size(); i++) {
				...
				if (exponentiationAsInt(input, exponentCandidate) != output) {
					matches = false;
				}
			}
	...
	private Set<Integer> getCandidateExponents () {
		...
		for (int i = 0; i < hypotheses.size(); i++) {
			...
			for (int j = 0; j < output; j++) {
				if (exponentiationAsInt(input, j) == output) {
	...
	private int exponentiationAsInt (final int input, final int j) {
		return (int) Math.pow(j,input);
	}
